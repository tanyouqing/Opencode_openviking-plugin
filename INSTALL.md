...
After the plugin is published to npm, normal users are recommended to enable it through OpenCode's package plugin mechanism:
...
The top-level `openviking.js` only forwards the first-level `.mjs` entry that OpenCode can discover to the actual plugin directory:
